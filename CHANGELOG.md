...
- Every `execute` action response now includes a plain-language top-level `message` field (`WithDisplayMessage`) -- a failure's `message` mirrors its existing `error` text, a success gets a per-action summary from a lookup table (falling back to "Done." for any action not in it). This is an experiment: HyperHQ currently shows a generic "completed" toast for every action regardless of the plugin's own `success`/`error` payload, and one of the two official example responses in HyperHQ's plugin docs uses a `message` field, suggesting the toast may read that field once present. Never overrides a `message` a handler already set itself.
- 5 new tests (105/105 passing): the canonicalArchetype same-run propagation scenario, and message-field coverage (success summary, failure mirrors error, handler-set message preserved, unmapped-action fallback).
